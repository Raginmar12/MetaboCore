# Checklist inicial NOM-004

## Estado
Borrador inicial. No exhaustivo.

Este documento no declara cumplimiento completo de la NOM-004. Su objetivo es servir como lista de vigilancia normativa durante el diseño progresivo de MetaboCore.

## Identificación del paciente
- [x] Formato operativo inicial para datos mínimos de identificación del paciente.
- [ ] Validar datos mínimos definitivos contra implementación futura.
- [ ] Mecanismo para evitar duplicidad de pacientes.
- [ ] Manejo de datos sensibles.

## Historia clínica
- [ ] Antecedentes heredofamiliares.
- [ ] Antecedentes personales patológicos.
- [ ] Antecedentes personales no patológicos.
- [ ] Padecimiento actual.
- [ ] Interrogatorio por aparatos y sistemas cuando aplique.
- [ ] Exploración física.
- [ ] Resultados previos y actuales de estudios.
- [ ] Diagnósticos o problemas clínicos.
- [ ] Pronóstico.
- [ ] Plan terapéutico.

## Notas médicas
- [ ] Nota inicial cuando aplique.
- [ ] Nota de evolución.
- [ ] Nota de interconsulta, si aplica.
- [ ] Nota de referencia o traslado, si aplica.

## Documentos clínicos
- [ ] Receta médica.
- [ ] Solicitud de estudios.
- [ ] Consentimiento informado cuando aplique.
- [ ] Indicaciones médicas.
- [ ] Resumen clínico cuando aplique.

## Seguridad y confidencialidad
- [ ] Control de acceso.
- [ ] Registro de autoría.
- [ ] Confidencialidad.
- [ ] Respaldo.
- [ ] Conservación.
- [ ] Trazabilidad de cambios.

## Fuente oficial
La fuente oficial de la NOM-004 ya se encuentra integrada en `docs/03_capa_nom/nom_004/fuente_oficial/`.

- [x] Fuente oficial integrada manualmente en `fuente_oficial/`.

## Pendientes
- [ ] Revisar numerales específicos de la NOM-004.
- [ ] Mapear cada requisito contra formatos actuales.
- [ ] Definir documentos mínimos para consulta ambulatoria MetaboCare.
- [ ] Definir requisitos futuros para expediente electrónico.
